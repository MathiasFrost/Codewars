...
public static class NodeCreator
{
	...
	public static Node CreateChain(int tail, int loop)
	{
		if (tail < 1 || loop < 1)
		{
			throw new IndexOutOfRangeException("Has to be greater than 0");
		}
		...
		{
			tailNodes.Add(new Node(((char)(1 + i + 'A')).ToString()));
		}

		for (var i = 0; i < tail - 2; i++)
		{
			tailNodes[i].Next = tailNodes[i + 1];
		}

		if (tail > 1)
		{
			root.Next = tailNodes[0];
		}

		IList<Node> loopNodes = new List<Node>();
		for (var i = 0; i < loop; i++)
		{
			loopNodes.Add(new Node((1 + i).ToString()));
		}

		for (var i = 0; i < loop - 1; i++)
		{
			loopNodes[i].Next = loopNodes[i + 1];
		}
